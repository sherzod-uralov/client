...
console.log(listTodo);
  ...
  <div className="flex justify-between items-center">
    ...
    <div className="flex items-center gap-3">
      <div className="flex items-center gap-1">
        ...
        <h3 className="font-light text-xs text-[#605e5c] dark:text-white">Sort</h3>
      </div>
      <div className="flex items-center gap-1">
        <GoLightBulb className="text-2xl text-[#605e5c] dark:text-white" />
        <h3 className="font-light text-xs text-[#605e5c] dark:text-white">Suggestion</h3>
      </div>
    </div>
  </div>
  ...
  >
    ...
    <button className= {`${todo == '' ? 'cursor-not-allowed' : 'cursor-pointer'}  px-[5px] py-[3px] border-solid dark:text-white dark:border-gray-50 border-gray-500 opacity-[0.3] border-[1.5px]`}>
      add
    </button>
  </div>
  <div className="mt-3 flex flex-col gap-2">
    {listTodo?.data?.List_todos?.map((e) => (
      <div  onContextMenu={(event) => clikedMenu(event, e.todo_id)} className="rounded-md relative shadow-md  bg-white dark:bg-[#252422] h-[55px] flex items-center" key={e.todo_id}>
